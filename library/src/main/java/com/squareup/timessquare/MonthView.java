...
public class MonthView extends LinearLayout {
	...
	public static MonthView create(ViewGroup parent, LayoutInflater inflater,
		...
		for (int offset = 0; offset < 7; offset++) {
			...
			final TextView textView = (TextView) headerRow.getChildAt(locale.getLanguage()
					.equals("en") ? offset : headerRow.getChildCount() - 1
					- offset);
		...
		for (int i = 0; i < 6; i++) {
			...
			if (i < numRows) {
				...
				for (int c = 0; c < week.size(); c++) {
					MonthCellDescriptor cell = week.get(locale.getLanguage().equals("en") ? c
							: week.size() - 1 - c);
